...
{
    // ------------------------------------------------------------------
    // Versione della parte server. Numerazione propria, indipendente da
    // quella del client: le due si aggiornano separatamente.
    // ------------------------------------------------------------------
    public static class Versione
    {
        public const string Numero = "1.2.0";
    }
    ...
    public class FormDettaglio : Form
    {
        ...
        private LeggiAvvio leggiAvvio;
        private CambiaAvvio cambiaAvvio;
        private CheckBox chkAvvio;
                             ...
                             LeggiAvvio leggi, CambiaAvvio cambia, CambiaSorv sorveglia)
        {
            ...
            // ultima voce: 20 per la riga in calce
            ClientSize = new Size(Largo, 16 + n * AltRiga + 82 + 20);
        ...
        {
            if (chkAvvio == null || leggiAvvio == null) return;
            sistemando = true;
            try { chkAvvio.Checked = leggiAvvio(); }
            catch { }
            sistemando = false;
        }
    ...
    public class Vassoio : ApplicationContext
    {
        ...
        private ToolStripMenuItem voceAvvio;
        ...
        // pianificata creata da Setup-Server.bat, che pero' per
        ...
        public Vassoio(bool apriSubito)
        {
            ...
            menu.Opening += delegate { voceAvvio.Checked = AvvioAutomaticoAttivo(); };
        ...
        private void Uno(string verbo, string task)
        {
            ...
            {
                foreach (Applicativo a in applicativi)
                    if (a.Task == task) { Comanda(verbo, a); break; }
